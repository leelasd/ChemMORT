...
class BaseModel(ABC):
    ...
    def _input(self, with_features=False):
        ...
        with tf.device('/cpu:0'):
            if with_features:
                ...
                self.seq1 = tf.placeholder(tf.int32, (self.batch_size, None), name="seq1")
                self.seq1_len = tf.placeholder(tf.int32, (self.batch_size, 1), name="seq1_len")
                self.seq2 = tf.placeholder(tf.int32, (self.batch_size, None), name="seq2")
                self.seq2_len = tf.placeholder(tf.int32, (self.batch_size, 1), name="seq2_len")
                self.mol_features = tf.placeholder(tf.float32, (self.batch_size, self.num_features), name="mol_features")
                seq1 = self.seq1
                seq1_len = self.seq1_len
                seq2 = self.seq2
                seq2_len = self.seq2_len
                mol_features = self.mol_features
            else:
    ...
    def _compute_loss(self, logits):
        ...
        current_ts = tf.to_int32(tf.minimum(tf.shape(self.shifted_target_seq)[1], tf.shape(logits)[1]))
        self.shifted_target_seq = tf.slice(self.shifted_target_seq, begin=[0, 0], size=[-1, current_ts])
        logits = tf.slice(logits, begin=[0, 0, 0], size=[-1, current_ts, -1])

        crossent = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=self.shifted_target_seq,
            logits=logits)
        self.target_mask = tf.slice(self.target_mask, begin=[0, 0], size=[-1, current_ts])
    ...
    def _compute_accuracy(self, prediction):
        ...
        current_ts = tf.to_int32(tf.minimum(tf.shape(self.shifted_target_seq)[1], tf.shape(prediction)[1]))
        self.shifted_target_seq = tf.slice(self.shifted_target_seq, begin=[0, 0], size=[-1, current_ts])
        prediction = tf.slice(prediction, begin=[0, 0], size=[-1, current_ts])
    ...
    def train(self, sess):
        ...
        if len(self.data) == 4:
            _, step = sess.run([self.train_step, self.global_step],{self.seq1:self.data[0],
                                                                self.seq1_len:self.data[1],
                                                                self.seq2:self.data[2],
                                                                self.seq2_len:self.data[3]}  )
        elif len(self.data) == 5:
                    _, step = sess.run([self.train_step, self.global_step], {self.seq1: self.data[0],
                                                                     self.seq1_len: self.data[1],
                                                                     self.seq2: self.data[2],
                                                                     self.seq2_len: self.data[3],
                                                                     self.mol_features: self.data[4]})
        else: raise Exception('请输入输入序列，输入序列长度，输出序列，输出序列长度，分子特征的前四个或者全部')
    ...
    def eval(self, sess):
        ...
        self.data = self.iterator.get_next()
        if len(self.data)==4:
            return sess.run(list(self.measures_to_log.values()),{self.seq1:self.data[0],
                                                                self.seq1_len:self.data[1],
                                                                self.seq2:self.data[2],
                                                                self.seq2_len:self.data[3]} )
        if len(self.data) ==5:
...
class GRUSeq2SeqWithFeatures(GRUSeq2Seq):
    ...
    def _compute_loss(self, sequence_logits, features_predictions):
        """Method that calculates the loss function."""
        current_ts = tf.to_int32(tf.minimum(tf.shape(self.shifted_target_seq)[1], tf.shape(sequence_logits)[1]))
        self.shifted_target_seq = tf.slice(self.shifted_target_seq, begin=[0, 0], size=[-1, current_ts])
        logits = tf.slice(sequence_logits, begin=[0, 0, 0], size=[-1, current_ts, -1])
