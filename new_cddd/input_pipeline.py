...
class InputPipeline():
    ...
    def get_next(self):
        l = len(self.dataset)
        if self.index>(l-self.batch_size):
            self.index = 0
        samples = self.dataset.iloc[self.index:(self.index+self.batch_size)]
        self.seq1 = samples['random_smiles'].tolist()
        self.seq2 = samples['canonical_smiles'].tolist()
        self.seq1,self.seq2,self.seq1_len,self.seq2_len = self._process_element(self.seq1,self.seq2)

        self.index = self.index+self.batch_size
        return self.seq1,self.seq1_len,self.seq2,self.seq2_len
    ...
    def _process_element(self, input_seq, output_seq):
        ...
        input_seq_max = max([len(seq) for seq in input_seq])+2
        output_seq_max = max([len(seq) for seq in output_seq])+2
        ins = np.zeros((self.batch_size,input_seq_max),dtype='int32')
        ins_len = np.ones((self.batch_size,1),dtype='int32')*2
        outs = np.zeros((self.batch_size, output_seq_max), dtype='int32')
        outs_len = np.ones((self.batch_size, 1), dtype='int32')*2
        for i in range(self.batch_size):
            input_seqa = self._char_to_idx(input_seq[i],
                                                   self.regex_pattern_input,
                                                   self.encode_vocabulary)
            output_seqa = self._char_to_idx(output_seq[i],
                                                    self.regex_pattern_output,
                                                    self.decode_vocabulary)
            input_seqa = self._pad_start_end_token(input_seqa, self.encode_vocabulary)
            output_seqa = self._pad_start_end_token(output_seqa, self.decode_vocabulary)

            input_seq_len = len(input_seqa)
            output_seq_len =len(output_seqa)
            input_seqa = input_seqa + (input_seq_max-input_seq_len)*[self.encode_vocabulary['</s>']]
            output_seqa = output_seqa + (output_seq_max - output_seq_len) * [self.decode_vocabulary['</s>']]
            ins[i,:] = np.array(input_seqa)
            outs[i,:] = np.array(output_seqa)
            ins_len[i,0] = input_seq_len
            outs_len[i,0] = output_seq_len
        return ins.astype('int32'),outs.astype('int32'),ins_len.astype('int32'),outs_len.astype('int32')
    ...
    def _pad_start_end_token(self, seq, vocabulary):
        ...
        seq = np.concatenate([np.array([vocabulary['<s>']]),
                             ...
                             """
        seq.insert(0, vocabulary['<s>'])
        seq.append(vocabulary['</s>'])
...
class InputPipelineWithFeatures(InputPipeline):
    ...
    def __init__(self, mode, hparams):
        ...
        Returns:
            None
        """
        super(InputPipelineWithFeatures, self).__init__(mode, hparams)
    ...
    def get_next(self):
        ...
        self.seq1 = samples['random_smiles'].tolist()
        self.seq2 = samples['canonical_smiles'].tolist()
        self.mol_features = np.array(samples[['Weight', 'logP(o/w)', 'apol','a_ICM','balabanJ', 'h_pavgQ', 'logS', 'mr', 'vdw_vol', 'zagreb']]).astype('float32')
        self.seq1, self.seq2, self.seq1_len, self.seq2_len = self._process_element(self.seq1, self.seq2)

        self.index = self.index + self.batch_size
        #print(self.mol_features.shape)
        return self.seq1,self.seq1_len,self.seq2, self.seq2_len, self.mol_features
...
class InputPipelineInferEncode():
    ...
    sequnces to the encoding part during inference time.
